...
class table implements i_type {
    ...
    public function edit_columns(form $form, $koperebielement) {
        ...
        if (isset($lines[0])) {
            ...
            foreach ($lines[0] as $id => $line) {
                echo "<fieldset><legend>" . get_string("table_edit_column", "local_kopere_bi")
    ...
    private function select_data($koperebielement, $collkey) {
        ...
        $types = [
            ["key" => "none", "value" => get_string("table_renderer_none", "local_kopere_bi")],
            ["key" => "string", "value" => "..."],
            ["key" => "number", "value" => get_string("table_renderer_number", "biblocks_table")],
            ["key" => "userfullname", "value" => get_string("table_renderer_userfullname", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_USERPHOTO, "value" => get_string("table_renderer_userphoto", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_VISIBLE, "value" => get_string("table_renderer_visible", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_STATUS, "value" => get_string("table_renderer_status", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_DATE, "value" => get_string("table_renderer_date", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_DATETIME, "value" => get_string("table_renderer_datetime", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_TIME, "value" => get_string("table_renderer_seconds", "local_kopere_bi")],
            ["key" => "translate", "value" => get_string("table_renderer_translate", "local_kopere_bi")],
            ["key" => table_header_item::RENDERER_FILESIZE, "value" => get_string("table_renderer_filesize", "local_kopere_bi")],
        ];
    ...
    public function preview($koperebielement) {
        ...
        foreach ($koperebielement->info_obj["column"] as $key => $column) {
            ...
            $name = string_util::get_string($koperebielement->info_obj["column"][$key]["title"]);
